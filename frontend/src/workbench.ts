...
export class WorkbenchGrid {
  readonly slots: WorkbenchSlot[];

  constructor(
    readonly rows: number = 3,
    readonly cols: number = 5,
    private readonly cellSpacing: number = 0.06,  // 60mm 间距
   ...
   */
  assign(subAssemblyRootId: string): WorkbenchSlot | null {
   ...
   */
  release(subAssemblyRootId: string): boolean {
   ...
   */
  findByOccupant(subAssemblyRootId: string): WorkbenchSlot | null {
