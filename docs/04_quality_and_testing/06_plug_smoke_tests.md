...
**Bug 4 已修 ✅**：底面 plug member halo silhouette。原 13 LDU 半径下，底面 port 中心嵌在 beam 板厚内（~3-4mm），depthTest:false 虽然让球壳"穿透"显示，但 silhouette 投影只露出薄薄一圈。半径提到 16 LDU（≈ 6.4mm），跨过常规板厚 → 底面 halo silhouette 多出 ~50% 可见面积。
...
3. 旋转 camera 看底面 → 底面 9 sibling 是否也都黄（现在用 16 LDU 应该明显）

**记录格式**：
```
hover 即显 18 arrows: __ Yes / __ No (复发)
sibling halo 命中率: __/10
底面 halo 可见: __ Yes / __ No (16 LDU bump 后期望 Yes)
```

**残留可调**（如果还痛）：
- 加大 port hit zone (`GIZMO_SPHERE_R_ENLARGED` 7→10 LDU)
- halo 半径 16→20 LDU 进一步加大（trade-off：相邻 plug 视觉糊在一起）
