...
- [x] **L51b PR-A：part-local COM 修正 + bbox 最低 vertex footprint**（推自 L51 v1）：staticsMath 的 `StabilityPart` 接 `quaternion` / `comLocal` / `bboxSize` / `bboxCenter`。⑤ 整体 COM 改为 `R_world · com_local + position` 加权累计 —— 偏心零件（电机壳、大齿轮）质心反映准确。④ Footprint 改为每 part 取 8 bbox 角点 → 转世界 → 取 Y-min 集合 → XZ 凸包，扁平大盘的真实支撑面被还原（v1 仅 part.position 单点，假阳性严重）。后端 `/api/get_verified_parts` 注入 `bbox_size` + `bbox_center`（直接读 port_lib_manager 持久化的 cached_data）。`PartCatalogEntry` 新增对应字段。模块级 scratch 池（_q / _v）避免 useMemo 重算时 alloc。新增 5 个 staticsMath 单测覆盖 ⑤ 旋转/退化 + ④ 扁平盘 footprint。**v1b 不做（留 PR-B/-C）**：反力求解（PR-B 含 ① + ②）/ 真应力 (PR-C，物理意义弱)。
