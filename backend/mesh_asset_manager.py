...
class MeshAssetManager:
    ...
    def __init__(self, cache_root: Optional[str] = None):
        if cache_root is None:
            ...
            self.mesh_cache_root = os.path.abspath(
                os.path.join(os.getcwd(), "data", "custom_assets")
            )
        else:
    ...
    def get_absolute_glb_path(
        self, part_id: str, color_code: int, cached_glb_path: Optional[str] = None
    ) -> str:
        """
        根据给定的零件 ID 和 缓存路径记录，计算其最终的本地绝对物理路径。
        """
        raw_path = cached_glb_path or self._get_default_glb_filename(
            part_id, color_code
        )
    ...
    def _compute_mesh_url(self, abs_path: str) -> str:
        ...
        try:
            ...
            rel_path = rel_path.replace(
                "\\", "/"
            )  # 必须强制将 Windows 斜杠转换为 URL 斜杠的标准化

            # 如果 relpath 指出该文件竟爬出了 cache_root (防呆设计保障 SRP)
            if rel_path.startswith(".."):
                logger.warning(
                    f"[MeshAssetManager] {abs_path} 的相对路径跳出了资源根目录, 降级截取 basename。"
                )
    ...
    def ensure_mesh_exists(
        ...
        color_code: int,
        geo_processor,
        cached_glb_path: Optional[str] = None,
    ) -> str:
        """
        ...
        if not os.path.exists(abs_path):
            logger.info(
                f"[MeshAssetManager] GLB 文件未命中物理缓存: {abs_path}，正在委托烘焙..."
            )
            try:
                os.makedirs(os.path.dirname(abs_path), exist_ok=True)
                # Ensure .dat extension is present for processor
                dat_filename = (
                    part_id if part_id.lower().endswith(".dat") else f"{part_id}.dat"
                )
                geo_processor.convert_to_glb(
                    dat_filename, abs_path, color_code=color_code
                )
