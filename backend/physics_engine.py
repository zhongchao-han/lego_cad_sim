...
class PhysicsEngine:
    ...
    def _configure_joints(self):
        ...
        for i in range(self.num_joints):
            ...
            # pybullet 的 joint type 常量没有 JOINT_CONTINUOUS（issue #87）；
            # URDF parser 把 <joint type="continuous"> 内部归为 unbounded JOINT_REVOLUTE，
            # 所以这里只判 REVOLUTE 一个常量即覆盖 URDF 的 continuous + revolute。
