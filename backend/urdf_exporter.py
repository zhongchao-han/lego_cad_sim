...
class URDFExporter:
    ...
    def export(
        ...
        for node_id, params in urdf_tree.nodes(data=True):
            ...
            collision_mesh = getattr(
                part_data, "collision_mesh", f"{node_id}_vhacd.obj"
            )
            ...
            ET.SubElement(
                inertial,
                "inertia",
                ixx=str(inertia_mat[0, 0]),
                ixy=str(inertia_mat[0, 1]),
                ixz=str(inertia_mat[0, 2]),
                iyy=str(inertia_mat[1, 1]),
                iyz=str(inertia_mat[1, 2]),
                izz=str(inertia_mat[2, 2]),
            )
            ...
            ET.SubElement(
                ET.SubElement(visual, "geometry"), "mesh", filename=visual_mesh
            )
            ...
            ET.SubElement(
                ET.SubElement(collision, "geometry"), "mesh", filename=collision_mesh
            )
        ...
        for u, v, params in urdf_tree.edges(data=True):
            ...
            joint = ET.SubElement(robot, "joint", name=f"joint_{u}_to_{v}", type=j_type)
