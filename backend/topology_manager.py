...
class TopologyManager:
    ...
    def connect_ports(self, edge: ConnectionEdge):
        """记录零件间的一个端口至端口的连接点"""
        if not self.graph.has_node(edge.parent_id) or not self.graph.has_node(
            edge.child_id
        ):
            logger.error(
                "连接建立失败：有端口引用的 PartNode ID 不在图中，请先 add_part。"
            )
            return

        self.graph.add_edge(
            edge.parent_id, edge.child_id, key=uuid.uuid4().hex, data=edge
        )
    ...
    def _calc_rel_transform(
        self, edge: "ConnectionEdge"
        ...
        for u, v in set((u, v) for u, v, _ in self.graph.edges):
            ...
            if edges_between_uv:
                ...
                if len(edge_list) > 1:
                    ...
                    logger.info(
                        f"过约束检测：零部件 {u} 与 {v} 间检测到 {len(edge_list)} 处端口碰撞/连接，合并为 Fixed Joint。"
                    )
        ...
        while queue:
            ...
            if current not in visited:
                ...
                if not urdf_tree.has_node(current):
                    urdf_tree.add_node(
                        current, data=simple_graph.nodes[current]["data"]
                    )
                ...
                for neighbor in simple_graph.successors(current):
                    ...
                    if neighbor not in visited:
                        ...
                        logger.warning(
                            f"闭环打破：移除由 {current} 到 {neighbor} 的环状物理连接，转至额外 Gazebo约束组."
                        )

        return urdf_tree

    def export_urdf(
        self, urdf_tree: nx.DiGraph, output_file: str = "lego_assembly.urdf"
    ):
        """
...
if __name__ == "__main__":
    ...
    def mk(name, ldraw_type, pos):
        item = Port.from_raw(
            name, ldraw_type, np.array(pos), id_rot, part_context="TestPart"
        )
        if item is None:
            raise ValueError(
                f"测试失败：无法创建类型为 {ldraw_type} 的测试端口。请在 port_semantics.py 中检查注册状况。"
            )
        return item
    ...
    e1 = ConnectionEdge(
        "A", "B", mk("p1", "peghole", [0.008, 0, 0]), mk("c1", "pin", [0, 0, -0.004])
    )
    e1_dup = ConnectionEdge(
        "A", "B", mk("p2", "peghole", [-0.008, 0, 0]), mk("c2", "pin", [0, 0, 0.004])
    )
    ...
    # B->C 正常传递：
    e2 = ConnectionEdge(
        "B", "C", mk("p", "axlehole", [0, 0, 0]), mk("c", "axle", [0, 0, 0])
    )
    manager.connect_ports(e2)

    # C->A 回归连接产生闭环测试：
    e3 = ConnectionEdge(
        "C", "A", mk("p", "pin", [0, 0.004, 0]), mk("c", "peghole", [0, -0.004, 0])
    )
