...
class GeometryProcessor:
        ...
        for line in lines:
            ...
            if line_type == '1' and len(parts) >= 15:
                ...
                try:
                    ...
                    local_mat = np.array([[a, b, c, x], [d, e_val, f, y], [g, h, i, z], [0, 0, 0, 1]])
    ...
    def discover_ports(self, filename: str, global_mat: np.ndarray = np.eye(4), root_id: Optional[str] = None) -> List[Dict[str, Any]]:
        """
        ...
        try:
            ...
            logger.error(f"Failed to discover ports from {filepath}: {err2}", exc_info=True)
            return []
        ...
        for line in lines:
            ...
            if child_file in SEMANTIC_PRIMITIVES or any(child_file.startswith(p) for p in CONNECTOR_PREFIXES):
                ...
                # 通用多单位步进元件 (连续轴、连续孔、连接插销等)
                    ...
                    base_unit_len = 1.0
                    for _k, _v in KNOWN_UNIT_LENGTHS.items():
                        if _k in child_file:
                            base_unit_len = _v; break

                    length_ldu = float(y_scale * base_unit_len * 20.0 if y_scale <= 10.0 else y_scale)
                    ...
                    for idx_k in range(num_units):
                        ...
                        if y_scale > 10.0:
                            # 基于中点 0.5 进行左右偏置
                            local_y = 0.5 + ((idx_k - num_units / 2.0 + 0.5) * 20.0 * step_dir) / y_scale
