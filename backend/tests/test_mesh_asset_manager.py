...
class TestMeshAssetManager:
    ...
    def test_get_absolute_glb_path(self, tmp_path):
        # 用 pytest tmp_path 取代硬编码 /mock_root：原写法在 Linux CI 下
        # MeshAssetManager.__init__ 的 os.makedirs 会因无权写入 /mock_root 而 PermissionError。
        mock_root = str(tmp_path / "mock_root")
        manager = MeshAssetManager(mock_root)

        # 测试缺省情况（生成目标位置）
        abs_path = manager.get_absolute_glb_path("3001.dat", 7)
        expected = os.path.abspath(os.path.join(mock_root, "3001_c7.glb"))
    ...
    def test_compute_mesh_url(self, tmp_path):
        ...
        # 1. 正常子文件
        normal_path = os.path.normpath(os.path.join(mock_root, "s", "3001_c7.glb"))
