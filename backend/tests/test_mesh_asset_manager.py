...
class TestMeshAssetManager:
    ...
    def test_get_absolute_glb_path(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            manager = MeshAssetManager(temp_dir)

            # 测试缺省情况（生成目标位置）
            abs_path = manager.get_absolute_glb_path("3001.dat", 7)
            expected = os.path.abspath(os.path.join(temp_dir, "3001_c7.glb"))

            # Windows 和 Linux 路径符号容错对比
            assert os.path.normpath(abs_path) == expected

            # 测试缓存已有绝对路径时的情况（跳过重算直接返回）
            mock_cached_abs = os.path.normpath(os.path.join(temp_dir, "model.glb"))
    ...
    def test_compute_mesh_url(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            # 1. 正常子文件
            normal_path = os.path.normpath(os.path.join(temp_dir, "s", "3001_c7.glb"))
            ...
            danger_path = os.path.normpath(os.path.join(temp_dir, "..", "outside_root", "3001_c7.glb"))
