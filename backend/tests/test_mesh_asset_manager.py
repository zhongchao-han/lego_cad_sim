...
class TestMeshAssetManager:
    ...
    def test_get_absolute_glb_path(self, tmpdir):
        manager = MeshAssetManager(str(tmpdir))
        
        # 测试缺省情况（生成目标位置）
        abs_path = manager.get_absolute_glb_path("3001.dat", 7)
        expected = os.path.abspath(os.path.join(str(tmpdir), "3001_c7.glb"))
    ...
    def test_compute_mesh_url(self, tmpdir):
        ...
        # 1. 正常子文件
        normal_path = os.path.normpath(os.path.join(str(tmpdir), "s", "3001_c7.glb"))
        ...
        danger_path = os.path.normpath(os.path.join(str(tmpdir), "..", "outside_root", "3001_c7.glb"))
