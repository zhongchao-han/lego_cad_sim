...
INTERFACE_REGISTRY: Dict[str, ConnectionInterface] = {
    ...
    "peghole": ConnectionInterface(
        Gender.FEMALE, Profile.CYLINDER, 6.0 * LDU, 20.0 * LDU
    ),
    "peghole.dat": ConnectionInterface(
        Gender.FEMALE, Profile.CYLINDER, 6.0 * LDU, 20.0 * LDU
    ),
    "beamhole.dat": ConnectionInterface(
        Gender.FEMALE, Profile.CYLINDER, 6.0 * LDU, 20.0 * LDU
    ),
    "connhole.dat": ConnectionInterface(
        Gender.FEMALE, Profile.CYLINDER, 6.0 * LDU, 20.0 * LDU
    ),
    "connhol.dat": ConnectionInterface(
        Gender.FEMALE, Profile.CYLINDER, 6.0 * LDU, 20.0 * LDU
    ),
    "npeghol.dat": ConnectionInterface(
        Gender.FEMALE, Profile.CYLINDER, 6.0 * LDU, 20.0 * LDU
    ),
    # 十字轴孔：用于锁定轴的旋转
    "axlehole": ConnectionInterface(
        Gender.FEMALE, Profile.CROSS, 4.0 * LDU, 20.0 * LDU
    ),
    "axlehole.dat": ConnectionInterface(
        Gender.FEMALE, Profile.CROSS, 4.0 * LDU, 20.0 * LDU
    ),
    # 带销孔的连接件（部分乐高零件同时具备圆孔和十字孔功能）
    "axleholepin.dat": ConnectionInterface(
        Gender.FEMALE, Profile.CYLINDER, 6.0 * LDU, 20.0 * LDU
    ),
    # ── MALE（销/轴/凸起）────────────────────────────────────────────────────
    # 普通销（间隙配合）：半径 5.9 LDU < 孔半径 6.0 LDU，可自由旋转
    "peg": ConnectionInterface(Gender.MALE, Profile.CYLINDER, 5.9 * LDU, 40.0 * LDU),
    "peg.dat": ConnectionInterface(
        Gender.MALE, Profile.CYLINDER, 5.9 * LDU, 40.0 * LDU
    ),
    "pin": ConnectionInterface(Gender.MALE, Profile.CYLINDER, 5.9 * LDU, 40.0 * LDU),
    "pin.dat": ConnectionInterface(
        Gender.MALE, Profile.CYLINDER, 5.9 * LDU, 40.0 * LDU
    ),
    ...
    # 半销：长度为梁厚一半
    "halfpin.dat": ConnectionInterface(
        Gender.MALE, Profile.CYLINDER, 5.9 * LDU, 20.0 * LDU
    ),
    # Technic 销连接原件（connect.dat）：4274/3673 等半销的核心语义原件
    "connect.dat": ConnectionInterface(
        Gender.MALE, Profile.CYLINDER, 5.9 * LDU, 20.0 * LDU
    ),
    # 摩擦销（friction pin）：半径 6.2 LDU > 孔半径 6.0 LDU，形成摩擦配合
    "fric_pin.dat": ConnectionInterface(
        Gender.MALE, Profile.CYLINDER, 6.2 * LDU, 40.0 * LDU
    ),
    "pin_friction": ConnectionInterface(
        Gender.MALE, Profile.CYLINDER, 6.2 * LDU, 40.0 * LDU
    ),
    # 摩擦脊原件 (Friction Ridges)：广泛用于 Technic 长销系列
    "confric3.dat": ConnectionInterface(
        Gender.MALE, Profile.CYLINDER, 6.2 * LDU, 20.0 * LDU
    ),
    "confric5.dat": ConnectionInterface(
        Gender.MALE, Profile.CYLINDER, 6.2 * LDU, 20.0 * LDU
    ),
    "confric6.dat": ConnectionInterface(
        Gender.MALE, Profile.CYLINDER, 6.2 * LDU, 40.0 * LDU
    ),
    "confric8.dat": ConnectionInterface(
        Gender.MALE, Profile.CYLINDER, 6.2 * LDU, 40.0 * LDU
    ),
    # 十字轴：锁定旋转
    ...
    "tube10.dat": ConnectionInterface(
        Gender.FEMALE, Profile.STUD, 6.0 * LDU, 4.0 * LDU
    ),
}
...
def build_fit_result(
    ...
    can_fully_insert = (
        fit != FitType.BLOCKED
        and fit != FitType.INCOMPATIBLE
        and plug.depth >= effective_beam
    )

    delta = plug.radius - socket.radius
    interference_mm = round(delta * 1000, 3)
    interference_pct = round(
        (delta / socket.radius * 100) if socket.radius > 0 else 0.0, 1
    )
...
if __name__ == "__main__":
    ...
    print(
        f"[build_fit_result] can_insert={res['can_fully_insert']}, method={res['method']}"
    )
