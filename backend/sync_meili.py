...
# LDraw 零件库默认路径
LDRAW_PARTS_DIR = os.environ.get(
    "LDRAW_PARTS_ROOT",
    os.path.join(os.path.dirname(__file__), "..", "ldraw_lib", "parts"),
)
...
def get_part_name(part_id: str) -> str:
    ...
    if not os.path.exists(part_path):
        logger.debug(
            f"[DEBUG] get_part_name() 分支: 路径不存在 {part_path}，返回 {part_id}"
        )
        return part_id
    try:
        with open(part_path, "r", encoding="utf-8") as f:
            ...
            if first_line.startswith("0 "):
                ...
                return name
            logger.debug(
                f"[DEBUG] get_part_name() 分支: 首行不符合格式 '{first_line}'，返回 {part_id}"
            )
